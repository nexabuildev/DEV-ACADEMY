...
export default function ProfileForm({ initialName, email }: { initialName: string | null, email: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label className="text-xs font-mono text-zinc-400">Email (No editable)</label>
        <input
          type="email"
          disabled
          defaultValue={email}
          className="w-full bg-zinc-900/50 border border-zinc-800 p-3 rounded-xl text-zinc-500 outline-none mt-1 cursor-not-allowed"
        />
      </div>
      ...
      <div>
        ...
        <input
          ...
          required
          className="w-full bg-black border border-zinc-700 p-3 rounded-xl text-white outline-none focus:border-blue-500 mt-1 transition-colors"
        />
      </div>
